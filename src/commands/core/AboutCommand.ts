...
@ApplyOptions<BaseCommandOptions>({
    name: "about",
    aliases: ["aboutme"],
    description: Locales.Commands.Core.About_CommandDescription,
    extendedDescription: {
        usage: "",
    },
...
export default class AboutCommand extends BaseCommand {
    ...
    override async chatInputRun(interaction: CommandInteraction): Promise<void> {
        ...
        return interaction.reply({ embeds: [await this.__intialize(translate)] });
    }
    ...
    private async __intialize(translate: TFunction): Promise<BaseMessageEmbed> {
        return this.utils
            ...
            .setDescription(
                translate(Locales.Commands.Core.About_ResponseContent, {
                    username: bold(this.container.client.user.username),
                })
            );
    }
    ...
    async registerApplicationCommands(registry: ApplicationCommandRegistry): Promise<void> {
        ...
        const command = new SlashCommandBuilder()
            .setName(this.name)
            .setDescription(translate(translate(Locales.Commands.Core.About_CommandDescription)));
